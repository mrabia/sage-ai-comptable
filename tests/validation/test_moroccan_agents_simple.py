#!/usr/bin/env python3
"""
Test des agents comptables marocains (version simple sans Unicode)
"""
import os
import sys
...
def test_moroccan_agent_personas():
    ...
    try:
        ...
        print("[OK] Gestionnaire d'agents créé avec succès")
        ...
        for persona, agent_type in tests:
            if agent_type in prompts:
                ...
                if persona in prompt:
                    ...
                    print(f"[ERREUR] Persona {persona} ({agent_type}) manquante")
            else:
                print(f"[ERREUR] Agent {agent_type} non trouvé")
        ...
        for agent_type, prompt in prompts.items():
            ...
            if found_keywords:
                ...
                print(f"[ERREUR] Agent {agent_type}: Aucune specificite marocaine trouvee")
...
if __name__ == "__main__":
    ...
    print(f"   - Test personas marocaines: {'REUSSI' if success1 else 'ECHOUE'}")
    print(f"   - Test initialisation: {'REUSSI' if success2 else 'ECHOUE'}")
